...
def load_mnist_trainval():
    ...
    train_data = data[:48000,].tolist()
    val_data = data[48000:,].tolist()
    train_label = label[:48000,].tolist()
    val_label = label[48000:,].tolist()
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    data_count = 0
    label_count = 0
    batch_data_count = len(data)
    batch_label_count = len(label)

    data = np.array(data)
    label = np.array(label)
        
    if shuffle == True:
        zips = list(zip(data, label))
        random.shuffle(zips)
        data, label = zip(*zips)
        np.array(data)
        np.array(label)
    ...
    for i in data:
        batched_datas.append(i)
        data_count += 1
        if data_count == batch_size:
            batched_data.append(np.array(batched_datas))
            batched_datas = []
            data_count = 0
            batch_data_count -= batch_size
        if batch_size > batch_data_count:
            if str(i) == str(data[len(data)-1]):
                batched_data.append(np.array(batched_datas))
                batched_datas = []
                data_count = 0    

    for i in label:
        batched_labels.append(i)
        label_count += 1
        if label_count == batch_size:
            batched_label.append(np.array(batched_labels))
            batched_labels = []
            label_count = 0
        if batch_size > batch_label_count:
            if str(i) == str(label[len(label)-1]):
                batched_label.append(np.array(batched_labels))
                batched_labels = []
                data_count = 0
