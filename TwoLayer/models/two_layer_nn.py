...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        A_d = A_d/y.shape[0]
        self.gradients['W2'] = a1.T.dot(A_d)
        self.gradients['b2'] = np.sum(A_d, axis=0)
        z_ds = A_d.dot(self.weights['W2'].T)
        z_d = np.multiply(z_ds,_baseNetwork.sigmoid_dev(self, z1))
        self.gradients['W1'] = X.T.dot(z_d)
        self.gradients['b1'] = np.sum(z_d, axis=0)
