...
class _baseNetwork:
    ...
    def softmax(self, scores):
        ...
        prob = [np.exp(scores[i])/np.sum(np.exp(scores[i])) for i in range(0,len(scores))]
    ...
    def cross_entropy_loss(self, x_pred, y):
        ...
        loss = np.sum(-np.log(x_pred[range(y.shape[0]),y]))/y.shape[0]
    ...
    def compute_accuracy(self, x_pred, y):
        ...
        acc = np.sum(np.array([np.exp(x_pred[i])/np.sum(np.exp(x_pred[i])) for i in range(0,len(x_pred))]).argmax(axis=1) == y)/len(y)
