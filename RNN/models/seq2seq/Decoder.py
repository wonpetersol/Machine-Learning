...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout = 0.2, model_type = "RNN"):
        ...
        if self.model_type == "RNN":
            self.rnn = nn.RNN(self.emb_size, self.decoder_hidden_size, dropout=dropout, batch_first=True)
        if self.model_type == "LSTM":
            self.lstm = nn.LSTM(self.emb_size, self.decoder_hidden_size, dropout=dropout, batch_first=True)

        self.fc1 = nn.Linear(self.decoder_hidden_size, self.output_size)
