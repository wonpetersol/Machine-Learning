...
class TransformerTranslator(nn.Module):
    ...
    def __init__(self, input_size, output_size, device, hidden_dim=128, num_heads=2, dim_feedforward=2048, dim_k=96, dim_v=96, dim_q=96, max_length=43):
        ...
        self.token_embedding = nn.Embedding(self.input_size, self.hidden_dim)
    ...
    def embed(self, inputs):
        ...
        embeddings = self.token_embedding(inputs) + self.position_embedding(torch.arange(inputs.shape[1]))
    ...
    def multi_head_attention(self, inputs):
        ...
        attention_one = torch.matmul(self.softmax(torch.matmul(self.q1(inputs),self.k1(inputs).transpose(-1,-2))/np.sqrt(self.dim_k)),self.v1(inputs))
        attention_two = torch.matmul(self.softmax(torch.matmul(self.q2(inputs),self.k2(inputs).transpose(-1,-2))/np.sqrt(self.dim_k)),self.v2(inputs))
        head = torch.cat((attention_one,attention_two),dim=2)
    ...
    def feedforward_layer(self, inputs):
        ...
        outputs =  self.fc2(self.reluLayer(self.fc1(inputs)))
